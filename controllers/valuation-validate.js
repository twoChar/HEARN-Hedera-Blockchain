...
async function validateValuation(req, res) {
  try {
    ...
    const {
      prj_id,
      valuation_company,
      valuation_time_period,
      total_asset_value,
      expected_revenue,
      expected_carbon_credits_value,
      total_valuation,
      valuation_date,
      transaction_id,
      transaction_hash,
    ...
    if (!proposal_hash) {
      return res.status(400).json({
        status: "error",
        message: "Missing required field: proposal_hash",
      });
    }

    // Step 1: Retrieve the latest proposal hash from the smart contract
    ...
    if (!decodedData || !Array.isArray(decodedData[0])) {
      return res.status(404).json({
        status: "error",
        message: "No proposal projects found in the contract",
      });
    }

    // Retrieve the latest proposal project from the contract
    const latestProject = decodedData[0][decodedData[0].length - 1];
    if (!latestProject) {
      return res.status(404).json({
        status: "error",
        message: "No latest proposal project found in the contract",
      });
    }
    ...
    const proposalDetailsString = JSON.stringify(proposalDataForHash);
    const calculatedHash = crypto
      .createHmac("sha256", secretKey)
      .update(proposalDetailsString)
      .digest("hex");

    // Step 3: Compare all three hashes and determine the result
    if (
      proposal_hash !== normalizedContractHash &&
      proposal_hash !== calculatedHash
    ) {
      return res.status(200).json({
        status: "failed",
        message: "proposal hash is tampered.",
        hashes: {
      ...
      calculatedHash !== proposal_hash ||
      calculatedHash !== normalizedContractHash
    ) {
      return res.status(200).json({
        status: "failed",
        message: "proposal data is tampered.",
        hashes: {
          provided_hash: proposal_hash,
          contract_hash: normalizedContractHash,
          calculated_hash: calculatedHash,
      ...
      return res.status(200).json({
        status: "success",
        message: "proposal hash successfully validated.",
      });
    }
  } catch (error) {
    console.error("Error validating proposal hash:", error.message);
...
module.exports = { validateValuation };
